...
export default function Discover() {
  ...
  const [newJob, setNewJob] = useState({ company: '', job_title: '', location: '', salary: '', status: 'saved', description: '', notes: '' });
  ...
  const handleAddJob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewJob({ company: '', job_title: '', location: '', salary: '', status: 'saved', description: '', notes: '' });
      fetchJobs();
    } catch (err) {
  ...
  const handleDropOnColumn = async (status: string) => {
    ...
    try {
      await apiClient.put(`/companies/${draggedJobId}/`, { status });
      if (viewDetailsJob && viewDetailsJob.id === draggedJobId) {
    ...
    {TRACKER_STATUSES.map(status => {
      ...
      return (
        <div
          ...
          className={`bg-gray-100/70 border rounded-2xl p-4 min-w-[320px] w-full max-w-sm flex-shrink-0 transition ${
          ...
          <div className="space-y-4">
            ...
            {jobsForColumn.map(job => (
              <div
                ...
                className={`bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition relative group cursor-grab active:cursor-grabbing ${
                  draggedJobId === job.id ? 'opacity-60 ring-2 ring-blue-300' : ''
                }`}
              >
                <button
                ...
                <div className="pt-3 border-t border-gray-100 flex gap-2 items-center">
                  <select
                    ...
                    className="flex-1 bg-gray-50 border border-gray-200 text-xs rounded-lg p-1.5 text-gray-800"
                  >
                    {TRACKER_STATUSES.map(s => (
                      <option key={s} value={s}>
                        Move to {formatStatusLabel(s)}
                      </option>
                    ))}
                  </select>
            ...
            {jobsForColumn.length === 0 && (
              <div className="text-center py-8 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-xl">
                {(searchQuery || filterCompany || filterLocation || filterSalary)
                  ? "No matches found"
                  : "No applications"}
              </div>
            )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
           <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleAddJob} className="space-y-4">
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Company</label><input required type="text" className="w-full border p-2.5 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 focus:bg-white" value={newJob.company} onChange={e => setNewJob({...newJob, company: e.target.value})} /></div>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4 border border-gray-200 p-4 rounded-2xl shadow-sm">
                  <div className="w-16 h-16 bg-indigo-500 rounded-xl flex items-center justify-center text-white text-2xl font-bold shadow-md">
                    {viewDetailsJob.company.charAt(0)}
                  </div>
                  <div>
                    ...
                    <p className="text-gray-600 font-medium flex items-center gap-2 mt-1">
                      <span>🏢 {viewDetailsJob.company}</span> • <span>📍 {viewDetailsJob.location}</span>
                    </p>
                  </div>
                </div>
              </div>
